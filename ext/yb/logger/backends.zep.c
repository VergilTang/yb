...
ZEPHIR_INIT_CLASS(Yb_Logger_Backends) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Yb\\Logger, Backends, yb, logger_backends, yb_logger_loggerabstract_ce, yb_logger_backends_method_entry, 0);

	zend_declare_property_null(yb_logger_backends_ce, SL("globalBackends"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_logger_backends_ce, SL("backends"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Yb_Logger_Backends, addBackend) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "spl_object_hash", NULL, 70, backend);
	...
	ZEPHIR_SINIT_VAR(_4);
	ZVAL_LONG(&_4, 1);
	ZEPHIR_CALL_FUNCTION(&_5, "preg_split", NULL, 71, &_3, level, &_4);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Logger_Backends, log) {
	...
	if (Z_TYPE_P(backends) == IS_ARRAY) {
		zephir_is_iterable(backends, &_1$$3, &_0$$3, 0, 0, "yb/logger/backends.zep", 35);
		for (
	...
	if (_4) {
		zephir_is_iterable(backends, &_6$$5, &_5$$5, 0, 0, "yb/logger/backends.zep", 41);
		for (
