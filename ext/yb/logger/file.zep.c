...
ZEPHIR_INIT_CLASS(Yb_Logger_File) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Yb\\Logger, File, yb, logger_file, yb_logger_loggerabstract_ce, yb_logger_file_method_entry, 0);

	zend_declare_property_null(yb_logger_file_ce, SL("logs"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_logger_file_ce, SL("path"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_long(yb_logger_file_ce, SL("chunkSize"), -1, ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Yb_Logger_File, log) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_STRING(&_0, "c", 0);
	ZEPHIR_CALL_FUNCTION(&_1, "date", NULL, 72, &_0);
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&_2, yb_std_ce, "tr", &_3, 73, message, context);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Logger_File, flush) {
	...
	ZEPHIR_SINIT_VAR(_4);
	ZVAL_LONG(&_4, (2 | 8));
	ZEPHIR_CALL_FUNCTION(&_5, "file_put_contents", NULL, 74, _1, _2, &_4);
	zephir_check_call_status();
	if (unlikely(!zephir_is_true(_5))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_6$$4, "yb/logger/file.zep", 47 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
