...
ZEPHIR_INIT_CLASS(Yb_View_Content) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\View, Content, yb, view_content, yb_view_content_method_entry, 0);

	zend_class_implements(yb_view_content_ce TSRMLS_CC, 1, yb_view_viewinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Yb_View_Content, run) {
	...
	if (zephir_fast_strlen_ev(contentType) > 0) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", NULL, 149, _8$$3);
		zephir_check_call_status();
	}
