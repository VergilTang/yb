...
ZEPHIR_INIT_CLASS(Yb_View_Json) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\View, Json, yb, view_json, yb_view_json_method_entry, 0);

	zend_class_implements(yb_view_json_ce TSRMLS_CC, 1, yb_view_viewinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Yb_View_Json, run) {
	...
	if (jsonCors) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 115, &_6$$3);
		zephir_check_call_status();
	}
	if (jsonHeaderPlain) {
		ZEPHIR_SINIT_VAR(_8$$4);
		ZVAL_STRING(&_8$$4, "Content-type: text/plain; charset=UTF-8", 0);
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 115, &_8$$4);
		zephir_check_call_status();
	}
