...
ZEPHIR_INIT_CLASS(Yb_View_Tpl) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\View, Tpl, yb, view_tpl, yb_view_tpl_method_entry, 0);

	zend_class_implements(yb_view_tpl_ce TSRMLS_CC, 1, yb_view_viewinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Yb_View_Tpl, run) {
	...
	if (unlikely(zephir_fast_strlen_ev(tplBaseDirectory) < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_view_exception_ce, "Missing option: tplBaseDirectory", "yb/view/tpl.zep", 13);
		return;
	}
	ZEPHIR_INIT_VAR(_7);
	...
	ZEPHIR_CALL_CE_STATIC(&_6, yb_std_ce, "valueat", &_1, 5, options, _2, _7);
	zephir_check_temp_parameter(_2);
	zephir_check_call_status();
	zephir_get_strval(_8, _6);
	ZEPHIR_CPY_WRT(tplLayoutDirectory, _8);
	if (unlikely(zephir_fast_strlen_ev(tplLayoutDirectory) < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_view_exception_ce, "Invalid option: tplLayoutDirectory", "yb/view/tpl.zep", 18);
		return;
	}
	ZEPHIR_INIT_NVAR(_2);
	...
	ZEPHIR_INIT_NVAR(_3);
	ZVAL_STRING(_3, "", ZEPHIR_TEMP_PARAM_COPY);
	ZEPHIR_CALL_CE_STATIC(&_10, yb_std_ce, "valueat", &_1, 5, options, _2, _3);
	zephir_check_temp_parameter(_2);
	zephir_check_temp_parameter(_3);
	zephir_check_call_status();
	...
	if (unlikely(_13)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_view_exception_ce, "Invalid option: tplId", "yb/view/tpl.zep", 23);
		return;
	}
	ZEPHIR_INIT_NVAR(_3);
	...
	ZEPHIR_CALL_CE_STATIC(&_15, yb_std_ce, "valueat", &_1, 5, options, _3, _16);
	zephir_check_temp_parameter(_3);
	...
	ZEPHIR_CALL_CE_STATIC(&_18, yb_std_ce, "valueat", &_1, 5, options, _3, _16);
	zephir_check_temp_parameter(_3);
	zephir_check_temp_parameter(_16);
	zephir_check_call_status();
	...
	if (unlikely(!((zephir_file_exists(path TSRMLS_CC) == SUCCESS)))) {
		...
		object_init_ex(_24$$6, yb_view_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_24$$6, "yb/view/tpl.zep", 31 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (zephir_fast_strlen_ev(tplLayout) > 0) {
		...
		if (unlikely(!ZEPHIR_IS_FALSE_IDENTICAL(_28$$7))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_view_exception_ce, "Invalid option: tplLayout", "yb/view/tpl.zep", 36);
			return;
		}
		...
		if (unlikely(!((zephir_file_exists(path TSRMLS_CC) == SUCCESS)))) {
			...
			object_init_ex(_32$$9, yb_view_exception_ce);
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_32$$9, "yb/view/tpl.zep", 41 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
	ZEPHIR_CALL_CE_STATIC(&_34, yb_std_ce, "renderscript", &_35, 117, path, data);
	zephir_check_call_status();
