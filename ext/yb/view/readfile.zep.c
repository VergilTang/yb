...
PHP_METHOD(Yb_View_ReadFile, run) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "header", &_12, 113, &_14);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "header", &_12, 113, &_14);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_15, "readfile", NULL, 117, readFile);
	zephir_check_call_status();
	if (unlikely(ZEPHIR_IS_FALSE_IDENTICAL(_15))) {
		ZEPHIR_INIT_VAR(_16$$7);
		object_init_ex(_16$$7, yb_view_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_16$$7, "yb/view/readfile.zep", 34 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
