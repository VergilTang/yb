...
PHP_METHOD(Yb_View_ReadFile, run) {
	...
	ZEPHIR_CALL_FUNCTION(&size, "filesize", NULL, 97, readFile);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "header", &_11, 93, _10);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(_2);
	...
	if (zephir_fast_strlen_ev(readFileContentType) > 0) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_11, 93, _15$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_SINIT_VAR(_16$$7);
		ZVAL_STRING(&_16$$7, "Content-Description: File Transfer", 0);
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_11, 93, &_16$$7);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_17, "readfile", NULL, 98, readFile);
	zephir_check_call_status();
