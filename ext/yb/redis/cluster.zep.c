...
PHP_METHOD(Yb_Redis_Cluster, __construct) {
	...
	if (zephir_is_true(slots)) {
		...
		ZEPHIR_CALL_CE_STATIC(&_0$$5, yb_std_ce, "valueat", &_1, 5, options, _2$$5, _3$$5);
		...
		ZVAL_LONG(_3$$5, 6379);
		ZEPHIR_CALL_CE_STATIC(&_5$$5, yb_std_ce, "valueat", &_1, 5, options, _2$$5, _3$$5);
		zephir_check_temp_parameter(_2$$5);
		zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_3, &_2)
		...
		if (zephir_instance_of_ev(result, yb_redis_errormoved_ce TSRMLS_CC)) {
			...
			ZVAL_BOOL(_12$$8, 1);
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "runcommandsbykeyinternally", &_13, 103, key, cmds, resultProcessor, _12$$8);
			zephir_check_call_status();
			RETURN_MM_NULL();
		}
		if (zephir_instance_of_ev(result, yb_redis_errorask_ce TSRMLS_CC)) {
...
PHP_METHOD(Yb_Redis_Cluster, newConnection) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 104, options);
	zephir_check_call_status();
	RETURN_MM();
