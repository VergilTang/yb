	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_3, &_2)
		...
		if (zephir_instance_of_ev(result, yb_redis_errormoved_ce TSRMLS_CC)) {
			...
			ZVAL_BOOL(_12$$8, 1);
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "runcommandsbykeyinternally", &_13, 91, key, cmds, resultProcessor, _12$$8);
			zephir_check_call_status();
			RETURN_MM_NULL();
		}
		if (zephir_instance_of_ev(result, yb_redis_errorask_ce TSRMLS_CC)) {
...
PHP_METHOD(Yb_Redis_Cluster, newConnection) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 92, options);
	zephir_check_call_status();
	RETURN_MM();
