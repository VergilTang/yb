...
PHP_METHOD(Yb_Image_Imagick, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, yb_image_imagick_ce, this_ptr, "__construct", &_2, 48, defaultOptions);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Yb_Image_Imagick, fromSize) {
	...
	if (width < 1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 57);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, fromPath) {
	...
	ZEPHIR_CALL_METHOD(NULL, imagick, "__construct", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_0, "realpath", NULL, 76, path);
	zephir_check_call_status();
	...
	if (!(!extension) && Z_STRLEN_P(extension)) {
		...
		ZVAL_LONG(&_6$$4, 4);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "pathinfo", NULL, 77, path, &_6$$4);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, captcha) {
	...
	if (width < 1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 124);
		return;
	}
	if (height < 1) {
		height = width;
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_42$$7, "mb_substr", &_43, 63, text, &_39$$7, &_40$$7, &_41$$7);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, largest) {
...
PHP_METHOD(Yb_Image_Imagick, resize) {
	...
	if (unlikely(width < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 256);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, crop) {
	...
	if (unlikely(_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Arguments x and y must not be negative", "yb/image/imagick.zep", 279);
		return;
	}
	if (unlikely(w < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument w must be positive", "yb/image/imagick.zep", 283);
		return;
	}
	if (h < 1) {
		h = w;
	...
	if (unlikely(_2)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Cropping out of area", "yb/image/imagick.zep", 291);
		return;
	}
	ZEPHIR_CALL_METHOD(&destIm, this_ptr, "fromimage", NULL, 0, im);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, thumbnail) {
	...
	if (unlikely(width < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 308);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, draw) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(_16, "yb/image/imagick.zep", 352 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
