...
PHP_METHOD(Yb_Image_Imagick, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, yb_image_imagick_ce, this_ptr, "__construct", &_2, 45, defaultOptions);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Yb_Image_Imagick, fromPath) {
	...
	ZEPHIR_CALL_METHOD(NULL, imagick, "__construct", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_0, "realpath", NULL, 72, path);
	zephir_check_call_status();
	...
	if (!(!extension) && Z_STRLEN_P(extension)) {
		...
		ZVAL_LONG(&_6$$4, 4);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "pathinfo", NULL, 47, path, &_6$$4);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, captcha) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_42$$7, "mb_substr", &_43, 59, text, &_39$$7, &_40$$7, &_41$$7);
		zephir_check_call_status();
