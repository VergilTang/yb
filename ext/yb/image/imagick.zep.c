...
PHP_METHOD(Yb_Image_Imagick, fromImage) {
	...
	ZEPHIR_CALL_METHOD(NULL, copyIm, "__construct", NULL, 9, this_ptr);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, fromPath) {
	...
	if (!(!extension) && Z_STRLEN_P(extension)) {
		...
		ZVAL_LONG(&_6$$4, 4);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "pathinfo", NULL, 12, path, &_6$$4);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, captcha) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_42$$7, "mb_substr", &_43, 25, text, &_39$$7, &_40$$7, &_41$$7);
		zephir_check_call_status();
