...
PHP_METHOD(Yb_Image_Imagick, text) {
	...
	zephir_array_fetch_string(&_3, m, SL("textWidth"), PH_NOISY | PH_READONLY, "yb/image/imagick.zep", 28 TSRMLS_CC);
	ZEPHIR_INIT_ZVAL_NREF(_4);
	ZVAL_LONG(_4, (zephir_get_numberval(_3) + padding2));
	zephir_update_property_zval(im, SL("width"), _4 TSRMLS_CC);
	zephir_array_fetch_string(&_5, m, SL("textHeight"), PH_NOISY | PH_READONLY, "yb/image/imagick.zep", 29 TSRMLS_CC);
	ZEPHIR_INIT_ZVAL_NREF(_4);
...
PHP_METHOD(Yb_Image_Imagick, fromImage) {
	...
	ZEPHIR_CALL_METHOD(NULL, copyIm, "__construct", NULL, 21, this_ptr);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, fromSize) {
	...
	if (width < 1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 55);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, fromPath) {
	...
	ZEPHIR_CALL_METHOD(NULL, imagick, "__construct", NULL, 0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_0, "realpath", NULL, 50, path);
	zephir_check_call_status();
	...
	if (!(!extension) && Z_STRLEN_P(extension)) {
		...
		ZVAL_LONG(&_6$$4, 4);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "pathinfo", NULL, 24, path, &_6$$4);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, captcha) {
	...
	if (width < 1) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 122);
		return;
	}
	if (height < 1) {
		height = width;
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_42$$7, "mb_substr", &_43, 37, text, &_39$$7, &_40$$7, &_41$$7);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, resize) {
	...
	if (unlikely(width < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 231);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, crop) {
	...
	if (unlikely(_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Arguments x and y must not be negative", "yb/image/imagick.zep", 254);
		return;
	}
	if (unlikely(w < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument w must be positive", "yb/image/imagick.zep", 258);
		return;
	}
	if (h < 1) {
		h = w;
	...
	if (unlikely(_2)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Cropping out of area", "yb/image/imagick.zep", 266);
		return;
	}
	ZEPHIR_CALL_METHOD(&destIm, this_ptr, "fromimage", NULL, 0, im);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Image_Imagick, thumbnail) {
	...
	if (unlikely(width < 1)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_image_exception_ce, "Argument width must be positive", "yb/image/imagick.zep", 283);
		return;
	}
	if (height < 1) {
		height = width;
...
PHP_METHOD(Yb_Image_Imagick, draw) {
	...
	ZEPHIR_CALL_METHOD(NULL, _16, "__construct", NULL, 1, _18);
	zephir_check_call_status();
	zephir_throw_exception_debug(_16, "yb/image/imagick.zep", 327 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
