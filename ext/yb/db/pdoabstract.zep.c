...
PHP_METHOD(Yb_Db_PdoAbstract, __construct) {
	...
	if (unlikely(!zephir_is_true(_1))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_db_exception_ce, "Missing extension: pdo", "yb/db/pdoabstract.zep", 10);
		return;
	}
	ZEPHIR_INIT_VAR(_2);
	...
	ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, 16, _3);
	zephir_check_temp_parameter(_3);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_5, "func_get_args", NULL, 17);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&_4, _2, "newinstanceargs", NULL, 18, _5);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Db_PdoAbstract, query) {
	...
	if (zephir_fast_count_int(params TSRMLS_CC) > 0) {
		zephir_is_iterable(params, &_3$$3, &_2$$3, 0, 0, "yb/db/pdoabstract.zep", 45);
		for (
	...
	ZEPHIR_CALL_CE_STATIC(&_12, yb_db_dbabstract_ce, "profiledquery", &_13, 19, sql, params, _14);
	zephir_check_call_status();
	...
	if (unlikely(!success)) {
		...
		object_init_ex(_16$$7, yb_db_queryexception_ce);
		zephir_array_fetch_long(&_17$$7, err, 2, PH_NOISY | PH_READONLY, "yb/db/pdoabstract.zep", 54 TSRMLS_CC);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_16$$7, "yb/db/pdoabstract.zep", 54 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	do {
		...
		if (mode == 4) {
			...
			while (1) {
				...
				zephir_array_append(&result, resultItem, PH_SEPARATE, "yb/db/pdoabstract.zep", 81);
			}
			RETURN_CCTOR(result);
		}
	} while(0);
	...
	object_init_ex(_14, yb_db_exception_ce);
	ZEPHIR_SINIT_VAR(_25);
	ZVAL_LONG(&_25, mode);
	ZEPHIR_CALL_FUNCTION(&_26, "strval", NULL, 20, &_25);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_27);
	ZEPHIR_CONCAT_SV(_27, "Invalid fetch mode: ", _26);
	ZEPHIR_CALL_METHOD(NULL, _14, "__construct", NULL, 2, _27);
	zephir_check_call_status();
	zephir_throw_exception_debug(_14, "yb/db/pdoabstract.zep", 86 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
