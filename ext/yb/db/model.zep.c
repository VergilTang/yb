...
ZEPHIR_INIT_CLASS(Yb_Db_Model) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\Db, Model, yb, db_model, yb_db_model_method_entry, 0);

	zend_declare_property_null(yb_db_model_ce, SL("db"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_db_model_ce, SL("table"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_db_model_ce, SL("primaryKey"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_db_model_ce, SL("autoIncrement"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Yb_Db_Model, newEntity) {
	...
	object_init_ex(entity, yb_db_entity_ce);
	ZEPHIR_INIT_VAR(_0);
	if (isNew) {
		ZVAL_BOOL(_0, 1);
	} else {
...
PHP_METHOD(Yb_Db_Model, chunkByDynamicWhere) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	long limit, c = 0;
	zval *processAndReturnNextWhere, *orderBy, *limit_param = NULL, *where = NULL, *collection = NULL, *_0$$4 = NULL, *_2$$4 = NULL;

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 1, &processAndReturnNextWhere, &orderBy, &limit_param);
	...
	while (1) {
		...
		ZEPHIR_CALL_METHOD(&collection, this_ptr, "all", &_1, 0, where, orderBy, _0$$4);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_ZVAL_FUNCTION(&where, processAndReturnNextWhere, NULL, 0, collection);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Db_Model, chunkByFixedWhere) {
	...
	zval *where = NULL;
	zval *delegate, *where_param = NULL, *orderBy = NULL, *limit_param = NULL, *collection = NULL, *_0$$4 = NULL, *_1$$4 = NULL, *_3$$4 = NULL, *_6$$4 = NULL;
	...
	while (1) {
		...
		_5$$4 = c < 1;
		if (!(_5$$4)) {
			ZEPHIR_CALL_ZVAL_FUNCTION(&_6$$4, delegate, NULL, 0, collection);
			zephir_check_call_status();
