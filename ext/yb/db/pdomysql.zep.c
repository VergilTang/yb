...
PHP_METHOD(Yb_Db_PdoMysql, insert) {
	...
	if (upsert) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$3, "preg_replace", NULL, 37, _2$$3, _3$$3, sql);
...
PHP_METHOD(Yb_Db_PdoMysql, upsert) {
	...
	if (Z_TYPE_P(primaryKey) == IS_ARRAY) {
		if (unlikely(!zephir_is_true(primaryKey))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_db_exception_ce, "Cannot upsert with empty where", "yb/db/pdomysql.zep", 35);
			return;
		}
		zephir_is_iterable(primaryKey, &_1$$3, &_0$$3, 0, 0, "yb/db/pdomysql.zep", 42);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_1$$3, &_0$$3)
			...
			if (unlikely(!(zephir_array_isset(data, k)))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(_3$$6, "yb/db/pdomysql.zep", 39 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
		...
		if (unlikely(!(zephir_array_isset(data, k)))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_7$$8, "yb/db/pdomysql.zep", 45 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
...
PHP_METHOD(Yb_Db_PdoMysql, countAndSelect) {
	...
	ZEPHIR_CALL_FUNCTION(&_4, "preg_replace", NULL, 37, _2, _3, s);
...
PHP_METHOD(Yb_Db_PdoMysql, paginationSql) {
...
PHP_METHOD(Yb_Db_PdoMysql, randomOrderSql) {
