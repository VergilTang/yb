...
PHP_METHOD(Yb_Db_Oci8, __construct) {
	...
	if (unlikely(!zephir_is_true(_1))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(yb_db_exception_ce, "Missing extension: oci8", "yb/db/oci8.zep", 12);
		return;
	}
	ZEPHIR_SINIT_NVAR(_0);
	ZVAL_STRING(&_0, "utf8", 0);
	ZEPHIR_CALL_FUNCTION(&oci, "oci_connect", NULL, 37, user, passwd, dsn, &_0);
	zephir_check_call_status();
	if (unlikely(!zephir_is_true(oci))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_2$$4, "yb/db/oci8.zep", 17 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Yb_Db_Oci8, query) {
	...
	ZEPHIR_CALL_FUNCTION(&statement, "oci_parse", NULL, 38, _1, sql);
	zephir_check_call_status();
	if (zephir_fast_count_int(params TSRMLS_CC) > 0) {
		zephir_is_iterable(params, &_3$$3, &_2$$3, 0, 0, "yb/db/oci8.zep", 48);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_3$$3, &_2$$3)
			...
			ZEPHIR_MAKE_REF(v);
			ZEPHIR_CALL_FUNCTION(NULL, "oci_bind_by_name", &_6, 39, statement, _5$$4, v);
			ZEPHIR_UNREF(v);
			zephir_check_call_status();
		}
	}
	...
	ZEPHIR_CALL_FUNCTION(&_8, "oci_execute", NULL, 40, statement, queryMode);
	zephir_check_call_status();
	success = zephir_get_boolval(_8);
	ZEPHIR_INIT_VAR(_10);
	ZVAL_DOUBLE(_10, startMt);
	ZEPHIR_CALL_CE_STATIC(&_8, yb_db_dbabstract_ce, "profiledquery", &_9, 16, sql, params, _10);
	zephir_check_call_status();
	...
	if (unlikely(!success)) {
		...
		ZEPHIR_CALL_FUNCTION(&err, "oci_error", NULL, 41, _12$$7);
		zephir_check_call_status();
		...
		object_init_ex(_14$$7, yb_db_queryexception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_14$$7, "yb/db/oci8.zep", 66 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	do {
		...
		if (mode == 1) {
			...
			while (1) {
				ZEPHIR_CALL_FUNCTION(&resultRow, "oci_fetch_array", &_16, 42, statement, queryMode);
				zephir_check_call_status();
				if (!(zephir_is_true(resultRow))) {
					break;
				}
				ZEPHIR_CALL_FUNCTION(&_17$$11, "array_change_key_case", &_18, 43, resultRow);
				zephir_check_call_status();
				zephir_array_append(&result, _17$$11, PH_SEPARATE, "yb/db/oci8.zep", 81);
			}
			RETURN_CCTOR(result);
		}
		if (mode == 2) {
			...
			ZEPHIR_CALL_FUNCTION(&resultRow, "oci_fetch_array", &_16, 42, statement, &_19$$13);
			zephir_check_call_status();
			if (zephir_is_true(resultRow)) {
				ZEPHIR_RETURN_CALL_FUNCTION("array_change_key_case", &_18, 43, resultRow);
				zephir_check_call_status();
				RETURN_MM();
			}
			RETURN_MM_NULL();
		...
		if (mode == 3) {
			...
			ZEPHIR_CALL_FUNCTION(&resultRow, "oci_fetch_array", &_16, 42, statement, &_20$$15);
			zephir_check_call_status();
		...
		if (mode == 4) {
			...
			while (1) {
				...
				zephir_array_append(&result, resultCell, PH_SEPARATE, "yb/db/oci8.zep", 107);
			}
			RETURN_CCTOR(result);
		}
	} while(0);
	...
	object_init_ex(_10, yb_db_exception_ce);
	ZEPHIR_SINIT_VAR(_25);
	ZVAL_LONG(&_25, mode);
	ZEPHIR_CALL_FUNCTION(&_26, "strval", NULL, 17, &_25);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_27);
	ZEPHIR_CONCAT_SV(_27, "Invalid fetch mode: ", _26);
	ZEPHIR_CALL_METHOD(NULL, _10, "__construct", NULL, 2, _27);
	zephir_check_call_status();
	zephir_throw_exception_debug(_10, "yb/db/oci8.zep", 112 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Yb_Db_Oci8, tryToCommit) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("oci_commit", NULL, 44, _0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Yb_Db_Oci8, tryToRollback) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("oci"), PH_NOISY_CC);
	ZEPHIR_RETURN_CALL_FUNCTION("oci_rollback", NULL, 45, _0);
	zephir_check_call_status();
	RETURN_MM();
