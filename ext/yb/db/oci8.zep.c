...
PHP_METHOD(Yb_Db_Oci8, query) {
	...
	ZEPHIR_CALL_FUNCTION(&s, "oci_parse", NULL, 41, _1, sql);
	zephir_check_call_status();
	...
	if (zephir_fast_count_int(params TSRMLS_CC) > 0) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_3$$3, &_2$$3)
			...
			ZEPHIR_MAKE_REF(v);
			ZEPHIR_CALL_FUNCTION(NULL, "oci_bind_by_name", &_6, 42, s, _5$$4, v);
			ZEPHIR_UNREF(v);
			zephir_check_call_status();
		}
	}
	...
	ZEPHIR_CALL_FUNCTION(&_8, "oci_execute", NULL, 43, s, m);
	zephir_check_call_status();
	...
	if (unlikely(!r)) {
		...
		ZEPHIR_CALL_FUNCTION(&e, "oci_error", NULL, 44, _11$$7);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Db_Oci8, queryAll) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&r, "oci_fetch_array", &_1, 45, _0$$3, m);
		zephir_check_call_status();
		if (!(zephir_is_true(r))) {
			break;
		}
		ZEPHIR_CALL_FUNCTION(&_2$$3, "array_change_key_case", &_3, 46, r);
		zephir_check_call_status();
...
PHP_METHOD(Yb_Db_Oci8, queryRow) {
	...
	ZEPHIR_SINIT_VAR(_1);
	ZVAL_LONG(&_1, ((4 + 8) + 1));
	ZEPHIR_CALL_FUNCTION(&r, "oci_fetch_array", NULL, 45, _0, &_1);
	zephir_check_call_status();
	if (zephir_is_true(r)) {
		ZEPHIR_RETURN_CALL_FUNCTION("array_change_key_case", NULL, 46, r);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_MM_RESTORE();
