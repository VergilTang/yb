...
PHP_METHOD(Yb_Std, sizeToBytes) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("floatval", NULL, 105, size);
...
PHP_METHOD(Yb_Std, bytesToSize) {
	...
	ZVAL_LONG(&_17, 0);
	ZEPHIR_RETURN_CALL_FUNCTION("strval", NULL, 106, &_17);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Yb_Std, newInstanceOf) {
	...
	ZEPHIR_CALL_FUNCTION(&a, "array_values", NULL, 103, args);
	zephir_check_call_status();
	do {
		if (c == 1) {
...
PHP_METHOD(Yb_Std, throwError) {
	...
	ZEPHIR_INIT_VAR(_3);
	ZVAL_LONG(_3, n);
	ZEPHIR_CALL_METHOD(NULL, _0, "__construct", NULL, 108, _2, _3);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Std, outputScript) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "extract", NULL, 109, data, _2, _3);
