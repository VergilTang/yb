...
PHP_METHOD(Yb_Upload_Uploader, __construct) {
	...
	ZEPHIR_SINIT_VAR(_4);
	ZVAL_LONG(&_4, 1);
	ZEPHIR_CALL_FUNCTION(&_5, "preg_split", NULL, 80, &_2, _1, &_3, &_4);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_6, "array_flip", NULL, 110, _5);
	zephir_check_call_status();
	zephir_update_property_this(this_ptr, SL("validExtensions"), _6 TSRMLS_CC);
	ZEPHIR_CALL_CE_STATIC(&_7, yb_std_ce, "sizetobytes", &_8, 111, maxSize);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Uploader, newUploadedFile) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 112, this_ptr);
	zephir_check_call_status();
	RETURN_MM();
