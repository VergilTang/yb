...
PHP_METHOD(Yb_Upload_Ftp, __destruct) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("ftp"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "ftp_close", NULL, 122, _0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Yb_Upload_Ftp, mkDirIfNotExists) {
	...
	ZEPHIR_SINIT_VAR(_3);
	ZVAL_LONG(&_3, 1);
	ZEPHIR_CALL_FUNCTION(&parts, "preg_split", NULL, 104, &_1, dir, &_2, &_3);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_1);
	ZVAL_STRING(&_1, "Yb\\Std::ignoreError", 0);
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 123, &_1);
	zephir_check_call_status();
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_5$$3, "ftp_chdir", &_6, 124, ftp, cur);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "restore_error_handler", NULL, 125);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_backwards_ex(_9, &_8)
		...
		ZEPHIR_CALL_FUNCTION(NULL, "ftp_mkdir", &_11, 126, ftp, cur);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
