...
PHP_METHOD(Yb_Upload_Ftp, __construct) {
	...
	if (ssl) {
		...
		ZEPHIR_CALL_FUNCTION(&ftp, "ftp_ssl_connect", NULL, 132, host, &_8$$3, &_9$$3);
		zephir_check_call_status();
	} else {
		...
		ZEPHIR_CALL_FUNCTION(&ftp, "ftp_connect", NULL, 133, host, &_10$$5, &_11$$5);
		zephir_check_call_status();
	}
	...
	if (_14) {
		ZEPHIR_CALL_FUNCTION(&_7, "ftp_login", NULL, 134, ftp, user, passwd);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_17, "ftp_pasv", NULL, 135, ftp, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Ftp, store) {
	...
	ZEPHIR_CALL_FUNCTION(&_4, "dirname", NULL, 126, destPath);
	zephir_check_call_status();
	...
	ZEPHIR_SINIT_VAR(_6);
	ZVAL_LONG(&_6, 2);
	ZEPHIR_CALL_FUNCTION(&_7, "ftp_put", NULL, 136, _5, destPath, source, &_6);
	zephir_check_call_status();
	if (zephir_is_true(_7)) {
		do {
			if (flag == 2 || flag == 1) {
				ZEPHIR_CALL_FUNCTION(NULL, "unlink", NULL, 43, source);
				zephir_check_call_status();
				break;
			}
		} while(0);
...
PHP_METHOD(Yb_Upload_Ftp, remove) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("ftp_delete", NULL, 137, _0, uri);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Yb_Upload_Ftp, exists) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("ftp"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(&_1, "ftp_mdtm", NULL, 138, _0, uri);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Ftp, __destruct) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("ftp"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "ftp_close", NULL, 139, _0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Yb_Upload_Ftp, mkDirIfNotExists) {
	...
	ZEPHIR_SINIT_VAR(_3);
	ZVAL_LONG(&_3, 1);
	ZEPHIR_CALL_FUNCTION(&parts, "preg_split", NULL, 27, &_1, dir, &_2, &_3);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_1);
	ZVAL_STRING(&_1, "Yb\\Std::ignoreError", 0);
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 140, &_1);
	zephir_check_call_status();
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_5$$3, "ftp_chdir", &_6, 141, ftp, cur);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "restore_error_handler", NULL, 142);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_backwards_ex(_9, &_8)
		...
		ZEPHIR_CALL_FUNCTION(NULL, "ftp_mkdir", &_11, 143, ftp, cur);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
