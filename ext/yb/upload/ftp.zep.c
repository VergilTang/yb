...
PHP_METHOD(Yb_Upload_Ftp, __construct) {
	...
	if (ssl) {
		...
		ZEPHIR_CALL_FUNCTION(&ftp, "ftp_ssl_connect", NULL, 114, host, &_8$$3, &_9$$3);
		zephir_check_call_status();
	} else {
		...
		ZEPHIR_CALL_FUNCTION(&ftp, "ftp_connect", NULL, 115, host, &_10$$5, &_11$$5);
		zephir_check_call_status();
	}
	...
	if (_14) {
		ZEPHIR_CALL_FUNCTION(&_7, "ftp_login", NULL, 116, ftp, user, passwd);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_17, "ftp_pasv", NULL, 117, ftp, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Ftp, store) {
	...
	ZEPHIR_CALL_FUNCTION(&_4, "dirname", NULL, 108, destPath);
	zephir_check_call_status();
	...
	ZEPHIR_SINIT_VAR(_6);
	ZVAL_LONG(&_6, 2);
	ZEPHIR_CALL_FUNCTION(&_7, "ftp_put", NULL, 118, _5, destPath, source, &_6);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Ftp, remove) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("ftp_delete", NULL, 119, _0, uri);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Yb_Upload_Ftp, __destruct) {
	...
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("ftp"), PH_NOISY_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "ftp_close", NULL, 120, _0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Yb_Upload_Ftp, mkDirIfNotExists) {
	...
	ZEPHIR_SINIT_VAR(_3);
	ZVAL_LONG(&_3, 1);
	ZEPHIR_CALL_FUNCTION(&parts, "preg_split", NULL, 101, &_1, dir, &_2, &_3);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_1);
	ZVAL_STRING(&_1, "Yb\\Std::ignoreError", 0);
	ZEPHIR_CALL_FUNCTION(NULL, "set_error_handler", NULL, 121, &_1);
	zephir_check_call_status();
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_5$$3, "ftp_chdir", &_6, 122, ftp, cur);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "restore_error_handler", NULL, 123);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_backwards_ex(_9, &_8)
		...
		ZEPHIR_CALL_FUNCTION(NULL, "ftp_mkdir", &_11, 124, ftp, cur);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
