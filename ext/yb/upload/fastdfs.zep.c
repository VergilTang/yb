...
PHP_METHOD(Yb_Upload_Fastdfs, store) {
	...
	ZEPHIR_INIT_VAR(_1);
	array_init(_1);
	ZEPHIR_CALL_FUNCTION(&destUri, "fastdfs_storage_upload_by_filename1", NULL, 113, source, extensionArg, _1, groupArg);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Upload_Fastdfs, remove) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("fastdfs_storage_delete_file1", NULL, 114, _0);
	zephir_check_call_status();
	RETURN_MM();
