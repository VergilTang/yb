...
PHP_METHOD(Yb_Loader_NamespaceDirectory, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, yb_loader_namespacedirectory_ce, this_ptr, "__construct", &_0, 89, _1);
...
PHP_METHOD(Yb_Loader_NamespaceDirectory, __invoke) {
	...
	while (1) {
		...
		ZVAL_STRING(&_1$$3, "\\", 0);
		ZEPHIR_CALL_FUNCTION(&pos, "strrpos", &_2, 91, match, &_1$$3);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_CE_STATIC(&_12, yb_loader_loaderabstract_ce, "isloaded", &_13, 90, name);
	zephir_check_call_status();
	if (unlikely(!zephir_is_true(_12))) {
		ZEPHIR_INIT_VAR(_14$$8);
		object_init_ex(_14$$8, yb_loader_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_14$$8, "yb/loader/namespacedirectory.zep", 51 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_MM_BOOL(1);
