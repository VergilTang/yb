...
PHP_METHOD(Yb_Router_UriPatterns, __construct) {
	...
	if (zephir_is_true(kvs)) {
		...
		ZEPHIR_CALL_FUNCTION(&_28$$7, "array_values", NULL, 21, kvs);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_29$$7, "preg_replace", NULL, 52, _27$$7, _28$$7, uri);
		zephir_check_call_status();
		zephir_get_strval(_30$$7, _29$$7);
		ZEPHIR_CPY_WRT(uri, _30$$7);
	}
	ZEPHIR_CALL_PARENT(NULL, yb_router_uripatterns_ce, this_ptr, "__construct", &_31, 104, uri);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
