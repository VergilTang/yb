...
ZEPHIR_INIT_CLASS(Yb_Factory_ClassNameAbstract) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\Factory, ClassNameAbstract, yb, factory_classnameabstract, yb_factory_classnameabstract_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	zend_declare_property_null(yb_factory_classnameabstract_ce, SL("args"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(yb_factory_classnameabstract_ce TSRMLS_CC, 1, yb_factory_factoryinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Yb_Factory_ClassNameAbstract, __get) {
	...
	if (unlikely(!(zephir_class_exists(className, 1 TSRMLS_CC)))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_2$$3, "yb/factory/classnameabstract.zep", 20 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	_5 = zephir_fetch_nproperty_this(this_ptr, SL("args"), PH_NOISY_CC);
	ZEPHIR_RETURN_CALL_CE_STATIC(yb_std_ce, "newinstanceof", &_4, 21, className, _5);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Yb_Factory_ClassNameAbstract, setArgs) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "func_get_args", NULL, 17);
	zephir_check_call_status();
...
PHP_METHOD(Yb_Factory_ClassNameAbstract, setArgArray) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "array_values", NULL, 22, args);
	zephir_check_call_status();
	zephir_update_property_this(this_ptr, SL("args"), _0 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
