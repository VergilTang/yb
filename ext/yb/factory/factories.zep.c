...
ZEPHIR_INIT_CLASS(Yb_Factory_Factories) {
	...
	ZEPHIR_REGISTER_CLASS(Yb\\Factory, Factories, yb, factory_factories, yb_factory_factories_method_entry, 0);

	zend_declare_property_null(yb_factory_factories_ce, SL("factories"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(yb_factory_factories_ce, SL("priorities"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_class_constant_long(yb_factory_factories_ce, SL("DEFAULT_PRIORITY"), 0 TSRMLS_CC);

	zend_class_implements(yb_factory_factories_ce TSRMLS_CC, 1, yb_factory_factoryinterface_ce);
	zend_class_implements(yb_factory_factories_ce TSRMLS_CC, 1, zend_ce_arrayaccess);
	return SUCCESS;
...
PHP_METHOD(Yb_Factory_Factories, set) {
	...
	ZVAL_LONG(_2, 1);
	ZEPHIR_MAKE_REF(_1);
	ZEPHIR_CALL_FUNCTION(NULL, "asort", NULL, 44, _1, _2);
	ZEPHIR_UNREF(_1);
...
PHP_METHOD(Yb_Factory_Factories, offsetGet) {
	...
	ZEPHIR_INIT_VAR(_1);
	object_init_ex(_1, yb_factory_exception_ce);
	ZEPHIR_INIT_VAR(_2);
	ZEPHIR_CONCAT_SV(_2, "Invalid index: ", index);
	ZEPHIR_CALL_METHOD(NULL, _1, "__construct", NULL, 2, _2);
	zephir_check_call_status();
	zephir_throw_exception_debug(_1, "yb/factory/factories.zep", 31 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
...
PHP_METHOD(Yb_Factory_Factories, __isset) {
	...
	if (zephir_fast_count_int(_0 TSRMLS_CC) > 0) {
		...
		zephir_is_iterable(_1$$3, &_4$$3, &_3$$3, 0, 0, "yb/factory/factories.zep", 56);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_4$$3, &_3$$3)
			...
			zephir_array_fetch(&_7$$4, _6$$4, index, PH_NOISY | PH_READONLY, "yb/factory/factories.zep", 51 TSRMLS_CC);
...
PHP_METHOD(Yb_Factory_Factories, __get) {
	...
	if (zephir_fast_count_int(_0 TSRMLS_CC) > 0) {
		_1$$3 = zephir_fetch_nproperty_this(this_ptr, SL("priorities"), PH_NOISY_CC);
		ZEPHIR_INIT_VAR(_2$$3);
		zephir_is_iterable(_1$$3, &_4$$3, &_3$$3, 0, 0, "yb/factory/factories.zep", 72);
		for (
		  ; zephir_hash_get_current_data_ex(_4$$3, (void**) &_5$$3, &_3$$3) == SUCCESS
		  ; zephir_hash_move_forward_ex(_4$$3, &_3$$3)
		) {
			...
			ZEPHIR_GET_HVALUE(_2$$3, _5$$3);
			_6$$4 = zephir_fetch_nproperty_this(this_ptr, SL("factories"), PH_NOISY_CC);
			zephir_array_fetch(&_7$$4, _6$$4, index, PH_NOISY | PH_READONLY, "yb/factory/factories.zep", 67 TSRMLS_CC);
			ZEPHIR_CPY_WRT(factory, _7$$4);
			ZEPHIR_CALL_METHOD(&_8$$4, factory, "__isset", NULL, 0, name);
			zephir_check_call_status();
			if (zephir_is_true(_8$$4)) {
	...
	ZEPHIR_INIT_VAR(_9);
	object_init_ex(_9, yb_factory_exception_ce);
	ZEPHIR_INIT_VAR(_10);
	ZEPHIR_CONCAT_SV(_10, "Invalid product: ", name);
	ZEPHIR_CALL_METHOD(NULL, _9, "__construct", NULL, 2, _10);
	zephir_check_call_status();
	zephir_throw_exception_debug(_9, "yb/factory/factories.zep", 74 TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
	return;
