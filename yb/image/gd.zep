...
class Gd extends ImageBackendAbstract
{
    ...
    public function save(<Image> im, string destPath) -> void
    {
        ...
        if destPath->length() > 0 {
            let destPathVar = destPath;
        } else {
            let destPathVar = null;
        }

        switch im->{"extension"} {
            case "png":
                if imagepng(im->{"handler"}, destPathVar) {
                    return;
                }
                break;

            case "gif":
                if imagegif(im->{"handler"}, destPathVar) {
                    return;
                }
                break;
            ...
            case "jpg":
            case "jpeg":
                if imagejpeg(im->{"handler"}, destPathVar, 100) {
                    return;
                }
                break;
